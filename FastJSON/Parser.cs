...
namespace FastJSON
{
    ...
    internal sealed class Parser
    {
        ...
        internal Parser(string json, bool AllowNonQuotedKeys)
        {
        ...
        private unsafe object ParseNumber(char* p)
        {
            ...
            return dob
                ? Double.Parse(UnsafeSubstring(p, startIndex, index - startIndex), NumberFormatInfo.InvariantInfo)
                : dec == false && index - startIndex < 20 && json[startIndex] != '-'
                ? Helper.CreateLong(json, startIndex, index - startIndex)
                : (object)Decimal.Parse(UnsafeSubstring(p, startIndex, index - startIndex), NumberFormatInfo.InvariantInfo);
        }

        private unsafe Token LookAhead(char* p) => lookAheadToken != Token.None ? lookAheadToken : (lookAheadToken = NextTokenCore(p));
